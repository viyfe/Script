 ...
 * @supported Quantumult X, Loon, Surge, Node.js(青龙)
 * @cron 15 8 * * *
 */
...
// 优先读取环境变量 PIGSEC_DATA 或 pigsec_data，都没有则读取这里的默认值
let accounts = (typeof process !== "undefined" && process.env.PIGSEC_DATA) ? process.env.PIGSEC_DATA : ($.getdata("pigsec_data") || "");

const USER_AGENT = "Mozilla/5.0 (iPhone; CPU iPhone OS 15_4_1 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Version/15.4 Mobile/15E148 Safari/604.1";
...
!(async () => {
    ...
    for (let i = 0; i < accountList.length; i++) {
        ...
        let pointsBefore = userInfoBefore.points || 0;
        ...
        if (isChecked) {
            ...
            let pointsAfter = userInfoAfter.points || pointsBefore;
    ...
    // 手机端原生通知（QX/Loon/Surge）
    $.msg($.name, "", notifyMsg.join("\n"));
    
    // 【新增】：如果是青龙环境，则调用 sendNotify 推送
    if ($.isNode() && notify) {
...
// --- Env 环境兼容类 (支持 QX / Loon / Surge / Node.js) ---
