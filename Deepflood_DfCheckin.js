...
if (isGetHeader) {
  ...
  if (!picked || Object.keys(picked).length === 0) {
    console.log("[DF] picked headers empty:", JSON.stringify(allHeaders));
    $notify("DF Headers 获取失败", "", "未获取到指定请求头，请重新再试一次。");
    $done({});
  } else {
    const ok = $prefs.setValueForKey(JSON.stringify(picked), DF_HEADER_KEY);
    console.log("[DF] saved picked headers:", JSON.stringify(picked));
    if (ok) {
      $notify("DF Headers 获取成功", "", "指定请求头已持久化保存。");
    } else {
      $notify("DF Headers 保存失败", "", "写入持久化存储失败，请检查配置。");
    }
    $done({});
  }
} else {
