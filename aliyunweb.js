 ...
 * 本文件以保留原始运行行为为目标，原脚本中可见的缺陷与风险也刻意保留。
 * 反混淆过程未执行原脚本，也未发起网络请求。
 */
...
class UserInfo {
  ...
  async doScene() {
    const csrfToken = this.token.match(new RegExp("c_csrf=([^;]*)"))[1];
    await this.getSceneList();
    await $.wait(this.getRandomTime());
    const sceneAvailable = await this.getSceneDetailPageInfoById();
    await $.wait(this.getRandomTime());
    if (sceneAvailable) {
      await this.getSceneStartPageInfoById();
      await $.wait(this.getRandomTime());
      if (resourceFrom === "2") {
        await this.startSceneById(csrfToken);
        await $.wait(this.getRandomTime());
        await this.closeSceneById(csrfToken);
        await $.wait(this.getRandomTime());
      } else {
        await this.doScene();
      }
  ...
  async getSceneList() {
    ...
    try {
      const pageNum = Math.floor(Math.random() * 26) + 1,
        pageSize = 21,
        requestOptions = {
        ...
        response = await this.fetch(requestOptions),
        scenes = response?.data?.list;
      if (scenes.length) {
        const scene = scenes[Math.floor(Math.random() * scenes.length)];
        sceneId = scene?.id, $.log("✅ 获取场景: " + scene.name + '[' + sceneId + ']');
      } else $.log("⛔️ 获取场景失败! " + e);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 获取场景失败! " + error);
    }
  }
  async getSceneDetailPageInfoById() {
    ...
    try {
      const requestOptions = {
        ...
        response = await this.fetch(requestOptions),
        buttonCode = response?.data?.developerAdcExperienceStatusVO?.buttonCode;
      return buttonCode ? buttonCode === '1' ? ($.log("✅ 确认场景状态: " + response?.data?.id), response?.data?.id) : ($.log("⛔️ 确认场景状态: " + response?.data?.id + " 已完成，将重新获取场景"), null) : ($.log('⛔️\x20确认场景状态:\x20' + response?.data?.id + " 状态异常，将重新获取场景"), null);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 确认场景状态失败! " + error);
    }
  }
  async getSceneStartPageInfoById() {
    ...
    try {
      const requestOptions = {
        ...
        response = await this.fetch(requestOptions);
      ip = response?.data?.ip;
      resourceFrom = response?.data?.resourceFrom.indexOf('1') > -1 ? '1' : '2';
      if (response?.data?.resourceCardInfoDTOList.length) {
        sectionId = response?.data?.resourceCardInfoDTOList[0]?.id;
      }
      $.log("✅ 获取场景初始化信息: " + sceneId);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 获取场景初始化信息失败! " + error);
    }
  }
  async startSceneById(csrfToken) {
    ...
    try {
      ...
      console.log((code === '200' ? '✅' : '⛔️') + " 开始场景: " + sceneId + ',\x20' + message);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 开始场景失败! " + error);
    }
  }
  async closeSceneById(csrfToken) {
    ...
    try {
      ...
      console.log((code === "200" ? '✅' : '⛔️') + " 结束场景: " + sceneId + ',\x20' + message);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 结束场景失败! " + error);
    }
  }
  async createResourceById(csrfToken) {
    ...
    try {
      const requestOptions = {
        ...
        response = await this.fetch(requestOptions);
      response?.data && console.log("✅ 开始创建场景资源: " + sceneId);
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 创建场景资源失败! " + error);
    }
  }
  async getResourceCardInfoById() {
    ...
    try {
      ...
      if (code === "200" && data) {
        if (data?.status !== "RUNNING") await $.wait(this.getRandomTime()), await this.getResourceCardInfoById();
        else return console.log("✅ 创建场景资源完毕: " + sceneId), true;
      }
    } catch (error) {
      this.ckStatus = false, $.log("⛔️ 创建场景资源失败! " + error);
    }
